...
class Wav2Vec2_ECAPA(pl.LightningModule):
        
    def __init__(self):
        super().__init__()
        self.out_score_file_name = None
        self.ssl_model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-large")
        self.ecapa_tdnn = ECAPA_TDNN(1024, lin_neurons=192)
...
def load_model(model_path, out_score_file_name):
    if model_path:
        ...
        model = Wav2Vec2_ECAPA.load_from_checkpoint(model_path, strict=True)
    else:
        model = Wav2Vec2_ECAPA(out_score_file_name)
